 ...
 * 
 */
public class ctrlWinReporteZoonotica extends GenericForwardComposer {

	// ----------------------------------------------------------------------------------------------------

	private Window winReporteZoonotica;
	...
	private String rutaJrxml = "/sigma/vistas/reportes/reportesJR/especialIncidenciasZoonosis/especialIncidenciasZoonosis.jrxml";
	...
	public ServicioNoModel getServicioNoModel() {
		return servicioNoModel;
	}

	public void setServicioNoModel(ServicioNoModel servicioNoModel) {
		this.servicioNoModel = servicioNoModel;
	}

	public Window getWinReporteZoonotica() {
		return winReporteZoonotica;
	}

	public void setWinReporteZoonotica(Window winReporteZoonotica) {
		this.winReporteZoonotica = winReporteZoonotica;
	}
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		servicioNoModel = (ServicioNoModel) SpringUtil
				.getBean("beanServicioNoModel");
		apagarBotones();
	...
	public void onClick$btnExportar() {
		...
		sqlcount = "select count(distinct paciente) from patologia, historial, historial_patologia, paciente, raza, especie where patologia.tipo_patologiaid = %d and  especie.id = %d and historial.id = historial_patologia.historialid and historial_patologia.patologiaid= patologia.id and raza.especieid = especie.id and paciente.razaid = raza.id and paciente.historia_medica = historial.pacientehistoria_medica and historial.fecha between '"
				+ HelperDate.format(dbInicio.getValue(), "yyyy-MM-dd")
				+ "' and '"
				+ HelperDate.format(dbFin.getValue(), "yyyy-MM-dd")
				+ "'";

		int tpid = 1;
		int caninoid = 1;
		int felinoid = 2;
		int canttcaninos = servicioNoModel.count(String.format(sqlcount, tpid,
				caninoid));
		int canttfelinos = servicioNoModel.count(String.format(sqlcount, tpid,
				felinoid));
		...
		parametros.put("titulo", "Reporte Incidencias Zoonóticas");
